...
export default class UserController {
  ...
  public static async createUser (ctx: BaseContext) {
    ...
    userToBeSaved.contactEmail = ctx.request.body.contactEmail;
    ...
    if (errors.length > 0) {
      ...
      ctx.status = 400;
      ctx.body = errors;
    } else if (await userRepository.findOne({ contactEmail: userToBeSaved.contactEmail })) {
  ...
  public static async updateUser (ctx: BaseContext) {
    ...
    userToBeUpdated.contactEmail = ctx.request.body.contactEmail;
    ...
    } else if (await userRepository.findOne({ id: Not(Equal(userToBeUpdated.id)),
      contactEmail: userToBeUpdated.contactEmail })) {
          // return BAD REQUEST status code and email already exists error
      ctx.status = 400;
      ctx.body = 'The specified e-mail address already exists';
    } else {
